...
contract KittenRaffle is ERC721, Ownable {
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        require(
            ...
            // Необходимо отправить достаточно для участия в розыгрыше
            "KittenRaffle:  It is necessary to send enough to participate in the drawing"
        );
    ...
    function refund(uint256 playerIndex) public {
        ...
        require(
            ...
            // Только игрок может получить возврат
            "KittenRaffle: Only the player can get a refund"
        );
        require(
            playerAddress != address(0),
            // Игрок уже получил возврат или не активен
            "KittenRaffle: The player has already received a refund or is inactive"
        );
    ...
    function selectWinner() public {
        require(
            ...
            // Розыгрыш еще не закончился
            "KittenRaffle: The drawing's not over yet"
        );
        require(
            players.length >= 4,
            // Необходимо минимум 4 игрока
        ...
        require(
            success,
            // Не удалось отправить призовой фонд победителю
            "KittenRaffle: Failed to send the prize money to the winner"
        );
    ...
    function withdrawFees() external {
        require(
            ...
            // В настоящее время активны игроки!
            "KittenRaffle: Currently active players!"
        );
        uint256 feesToWithdraw = totalFees;
        totalFees = 0;
        (bool success, ) = feeAddress.call{value: feesToWithdraw}("");
        // Не удалось вывести комиссию
        require(success, "KittenRaffle: Failed to withdraw commission");
    }
    ...
    function tokenURI(
        ...
        require(
            _exists(tokenId),
            "KittenRaffle: URI request for a non-existent token"
        );
